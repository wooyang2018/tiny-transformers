...
class PerformerAttention(nn.Module):
    ...
    def forward(self, x):
        k, q, v = torch.split(self.qkv_transform(x),
                              self.head_channels, dim=-1)
        ...
        y = torch.einsum('bti,bni->btn', qp, kptv) / \
            (D.repeat(1, 1, self.head_channels) + 1e-8)
...
class T2TViT(BaseTransformerModel):
    ...
    def __init__(self):
        ...
        pe = self._get_position_embedding(
            self.num_patches + 1, self.hidden_dim)
